<template>
  ...
  <skeletonTop :count="10" :loading="mvTopList.length">
    <section>
      <main v-for="(item,index) in mvTopList" :key="item.id" class="mvTopList" @click="goMvDetail(item.id)">
        <div :class="{active:index < 3}" style="width: 30px; font-size: 25px; margin-right: 10px; font-weight: 900;">
...
const mvTopList = ref([])
onMounted(async() => {
  const res = await getMvTopList(current.value)
