<template>
  <el-card>
    <div v-for="(item,index) in tags" :key="index" class="tags">
      <div style="margin-top: 10px;">{{ item.type }}:</div>
      <div
        v-for="(v,i) in item.tag"
        :key="i"
        class="item"
        :class="{active:params[item.params] === v.value}"
        @click="change(v)"
      >{{ v.name }}
      </div>
    </div>
  ...
  <skeleton :count="12" :loading="allMvData?.length">
    <section style="display: flex;justify-content: space-between;flex-wrap: wrap; overflow: auto;">
      <allMvCover :video-array="allMvData" />
    </section>
  </skeleton>
...
const tags = [
  {
    params: 'area',
    type: '地区',
    tag: [{ type: 0, value: '', name: '全部' }, { type: 0, value: '内地', name: '内地' }, {
      type: 0,
      value: '港台',
      name: '港台'
    }, { type: 0, value: '欧美', name: '欧美' }, { type: 0, value: '日本', name: '日本' }, { type: 0, value: '韩国', name: '韩国' }]
  },
  {
    params: 'type',
    type: '类型',
    tag: [{ type: 1, value: '', name: '全部' }, { type: 1, value: '官方版', name: '官方版' }, {
      type: 1,
      value: '原生',
      name: '原生'
    }, { type: 1, value: '现场版', name: '现场版' }, { type: 1, value: '网易出品', name: '网易出品' }]
  },
  {
    params: 'order',
    type: '排序',
    tag: [{ type: 2, value: '', name: '上升最快' }, { type: 2, value: '最热', name: '最热' }, { type: 2, value: '最新', name: '最新' }]
  }
]
</script>
