  ...
  <el-skeleton style="display: flex;justify-content: space-between; align-items: center;" :loading="!Boolean(banners.length)" :count="1" animated>
    ...
    <template #default>
      <el-carousel trigger="click" indicator-position="outside" type="card" height="230px">
        <el-carousel-item v-for="item in banners" :key="item.targetId" :label="item.typeTitle" @click="banner(item)">
          <el-image style="width:100%; height: 220px; border-radius: 5px;" :src="item.imageUrl" />
        </el-carousel-item>
      </el-carousel>
    </template>
  </el-skeleton>
